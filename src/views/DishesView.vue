    ...
    <form-block>
        <template #header>
            <RouterLink :to = "{name: 'users'}">
                <b-avatar text = "<=" variant = "primary"></b-avatar>
            </RouterLink>
            <h2>Введите позицию и стоимость</h2>
            <b-avatar :to = "{name: 'results'}" 
                text = "$"    
                :variant="isNorm ? 'primary' : 'secondary'"
                :disabled="!isNorm"></b-avatar>
        </template>
        <template #add-button>
            <b-button @click = "newUser.addDishForm(newUser.users.map((item)=>item.name))"
                variant = "secondary"
               >Добавить</b-button>
        </template>
        <template #body style = "margin-bottom: 20px !important; ">
            ...
            <div v-else  v-for = "(dish, index) in newUser.dishes" :key = "dish.id" class = "w-100 d-flex mb-3 justify-content-center "  v-bind = "isActiveBtn = false">
                <dish-form  :id = "dish.id" :dish = "dish" :index = "index" :isValid = "dish.isValid"></dish-form>
            </div>
        </template>
        <template
            #footer  v-if = "isActiveBtn"
        > </template>
        <template
            #footer v-else>
                <b-button
